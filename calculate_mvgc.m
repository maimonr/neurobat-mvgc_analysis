...
pnames = {'trialSelection','selectBat','winSize','stepSize','timeWin','modelOrder','averageType','subSample','nSub'};
dflts  = {'all',[],1,0.25,[-Inf,Inf],'AIC','tetrode','none',10};
[trialSelection,selectBat,winSize,stepSize,timeWin,mOrder_flag,averageType,subsampleFlag,nSub] = internal.stats.parseArgs(pnames,dflts,varargin{:});
...
if strcmp(subsampleFlag,'none')
    [FF, pval, sig] = deal(nan(nBat,nBat,nWin));
...
parfor win_k = 1:nWin
    ...
    if strcmp(subsampleFlag,'none')
        ...
        [A,SIG] = tsdata_to_var(Xwin,mOrder,regmode);
    ...
    for channel_k1 = 1:nBat
        ...
        for channel_k2 = 1:nBat
            ...
            switch subsampleFlag
                ...
                case 'none'
                    FF_current = autocov_to_mvgc(G,idx1,idx2);
                    
                    if isbad(FF_current,false)
                        fprintf(2,' *** skipping - GC calculation failed\n');
                        continue
                    end
                    
                    FF_win(channel_k1,channel_k2,:) = FF_current;
